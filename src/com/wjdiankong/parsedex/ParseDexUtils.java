...
public class ParseDexUtils {
    ...
    public static void parseFieldIds(byte[] srcByte) {
        ...
        for (FieldIdsItem item : fieldIdsList) {
            ...
            System.out.println("class:" + stringList.get(classIndex) + ",name:" + stringList.get(item.name_idx)
    ...
    public static void parseMapItemList(byte[] srcByte) {
        ...
        int size = Utils.byte2int(sizeByte);
        for (int i = 0; i < size; i++) {
            mapList.map_item.add(parseMapItem(
                    Utils.copyByte(srcByte, mapListOffset + 4 + i * MapItem.getSize(), MapItem.getSize())));
        }
    }
    ...
    private static String getString(byte[] srcByte, int startOff) {
        ...
        // 第二种方式
        /*
         * System.out.println("off:"+Utils.bytesToHexString(Utils.int2Byte(
         * startOff))); List<Byte> byteList = new ArrayList<Byte>();
         * //第一个字节是长度，所以过滤一下 byte b = srcByte[startOff+1]; int index = 0;
         * while(b != 0){ byteList.add(b); index++; b = srcByte[startOff+index];
         * } byte[] byteAry = new byte[byteList.size()+1]; for(int
